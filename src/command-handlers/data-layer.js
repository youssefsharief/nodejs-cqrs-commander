...
const handlerDataLayer = (aggregateId) => ({
    ...
    async saveCommandActionsToDb(newEvents, aggregateAfterApplingCommand) {
        await db.concurrencyCheck(this.aggregateVersion, aggregateId)
        await db.saveEvents(newEvents)
        if (newEvents.findIndex(e => e.sequence % 10 === 0) >= 0)
            await db.saveSnapshot({ lastEventSequence: this.eventSequence, aggregateRootId: aggregateId, payload: aggregateAfterApplingCommand })
    }
